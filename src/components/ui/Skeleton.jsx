...
export default function Skeleton( { className, ...props } ) {
	return (
		<div
			className={ cn( 'animate-pulse rounded-lg bg-warm-100', className ) }
			{ ...props }
		/>
	);
}
...
export function SkeletonCard( { className, height = 88 } ) {
	return <Skeleton className={ cn( 'border border-warm-200/70 bg-surface', className ) } style={ { height, borderRadius: 12 } } />;
}
...
export function TableSkeleton( { rows = 5 } ) {
	return (
		<div className="rounded-xl border border-warm-200/70 bg-surface overflow-hidden">
			<div className="flex gap-4 border-b border-warm-200/50 px-4 py-3">
				{ [ 120, 160, 80, 60, 70 ].map( ( w, i ) => <Skeleton key={ i } className="h-2.5" style={ { width: w } } /> ) }
			</div>
			{ Array.from( { length: rows }, ( _, i ) => (
				<div key={ i } className="flex items-center gap-4 border-b border-warm-200/30 px-4 py-3 last:border-b-0">
	...
	return (
		<div className="px-6 py-[22px]">
			...
			<div className="flex flex-col gap-1.5">
				{ [ 1, 2, 3 ].map( ( i ) => (
					<div key={ i } className="flex items-center gap-3 rounded-xl border border-warm-200/70 bg-surface px-4 py-3">
...
export function SettingsSkeleton() {
	return (
		<div className="mx-auto max-w-[680px] px-6 py-[22px]">
			...
			{ [ 1, 2, 3 ].map( ( i ) => (
				<div key={ i } className="mb-3 rounded-xl border border-warm-200/70 bg-surface p-5">
